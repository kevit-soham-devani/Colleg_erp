...
class StudentController {
    async createStudent(req, res, next) {
        try {
            ...
            const isSeatAvailable = await (0, student_helper_1.checkSeat)(department, batch, res, req);
            if (!isSeatAvailable) {
                return res.send({
            ...
            await (0, student_helper_1.incrementSeatCount)({ department, batch, res });
    ...
    async deleteStudent(req, res, next) {
        try {
            ...
            const student = await (0, student_DAL_1.deleteStudent)(_id);
            if (!student) {
                return res
                    .status(404)
                    .send({ message: 'Student does not exist' });
            }
            await (0, student_helper_1.decrementSeatCount)(student, res);
    ...
    async updateStudent(req, res, next) {
        try {
            ...
            const updatedStudent = await (0, student_DAL_1.updateStudent)(_id, updateData);
            if (!updatedStudent) {
